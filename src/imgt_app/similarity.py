...
def find_similar_tcrs(
    ...
    cand = df[df["v_b_gene"].map(_v_family) == fam]
    if len(cand) < 5:
        cand = df  # widen if the family is too thin
    total = len(cand)
    if total == 0:
        warnings.append(
            DossierWarning(
    ...
    scored = cand.assign(dist_score=dists, sim_score=1.0 - dists / max_d)
    scored = scored[scored["sim_score"] >= min_similarity].nsmallest(top_k, "dist_score")

    neigh = [
        Neighbour(
            cdr3_b_aa=r.cdr3_b_aa,
            v_b_gene=r.v_b_gene,
            j_b_gene=r.j_b_gene,

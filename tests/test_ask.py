...
def test_heuristic_scans_all_tokens_for_annotate(monkeypatch):
    monkeypatch.setattr(ask, "llm_available", lambda: False)
    resp = ask.answer(AskRequest(query="annotate CASSLGTEAFF"))
    assert resp.intent == "dossier"
    assert resp.dossier is not None
    assert resp.dossier.query_echo.get("value") == "CASSLGTEAFF"
